...
const UploadObservationPopup = ({ open, onClose, observationId, onUploadSuccess }) => {
  ...
  const handleUpload = () => {
    if (selectedFile) {
      ...
      axios
        .post("http://localhost:3000/upload/avance-observacion-pdf", formData, {
          headers: {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>Subir Archivo de Observación</DialogTitle>
      <DialogContent>
        {!selectedFile ? (
          <>
            <div
          ...
          <div style={{ marginTop: "20px", textAlign: "center" }}>
            <Typography variant="body1">
              Archivo seleccionado: {selectedFile.path} - {(selectedFile.size / (1024 * 1024)).toFixed(2)} MB
            </Typography>
            <Button
              variant="contained"
              color="secondary"
...
export default UploadObservationPopup;